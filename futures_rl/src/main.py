...
if __name__ == "__main__":
    ...
    env = FuturesEnv(path='/workspace/BTC_src/BTCUSDT-5m-total.csv')
    ...
    if choice == 1:
        model_name = input('모델 저장 이름을 입력해주세요. [default: ppo]: ')
        ...
        lr_actor = float(input('lr_actor [default: 1e-4]: ') or "1e-4")            
        lr_critic = float(input('lr_critic [default: 3e-4]: ') or "3e-4")
        gamma = float(input('gamma [default: 0.99]: ') or "0.99")
        epsilon = float(input('epsilon [default: 0.1]: ') or "0.1")
        ...
        if user_input.lower() == 'y':
            ...
            # 추가 학습 시작
            continue_training(env, latest_model, episodes)
        else:
            print("학습을 취소합니다.") 
