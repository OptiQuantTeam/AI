...
class FuturesEnv(gym.Env):
        ...
        if self.position != position_direction:
            ...
            if self.position != 0:
                ...
                if self.position_size != 0 and -position_direction * (current_price - self.entry_price) / self.entry_price > 0.7:
                    ...
                    reward = profit
                '''
                reward += self.position * (current_price - self.entry_price) / (self.entry_price * Minute) 
            ...
            if self.balance < self.initial_balance * 0.9:
                ...
                #reward += -1000 - self.num
                self.liquidated = True
            elif self.balance > self.initial_balance * 1.2:
                done = True
                self.sucess += 1
                reward += 10000000 - self.num
                self.clear = True
    ...
    def render(self):
        ...
        if not self.liquidated and not self.clear:
            ...
            print(f"  목표 달성 여부: {'지정한 목표 달성' if self.clear else '달성 실패'}")
            print(f"  현재 에피소드까지 달성한 목표 개수: {self.sucess}")
