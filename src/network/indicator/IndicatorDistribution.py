...
class IndicatorDistribution(nn.Module):
    ...
    def forward(self, state):
        ...
        # 2-1) EMA 기울기 분석
        ema_4_slope = state[:, 2]  # EMA_4 기울기
        ema_12_slope = state[:, 3]  # EMA_12 기울기
        ema_24_slope = state[:, 4]  # EMA_24 기울기
        ...
        stoch_rsi = state[:, 5:6]  # stochRSI 값
        ...
        # 중립 구간 + 밴드 폭 증가
        neutral_band_width = (stoch_rsi.squeeze(-1) >= 20) & (stoch_rsi.squeeze(-1) <= 80) & (bollinger_values[:, 0] > 0)
