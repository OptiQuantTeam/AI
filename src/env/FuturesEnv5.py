...
class FuturesEnv5(gym.Env):
    def __init__(self, path=None, logger=None):
        ...
        self.max_steps = 1000
        self.max_position_ratio = 0.3  # 최대 포지션 크기 비율 감소
        self.stop_loss_threshold = 0.012  # 손절매 임계값 감소
        self.trailing_stop = 0.006  # 트레일링 스탑 감소
    ...
    def step(self, action):
        ...
        if self.position != position_direction:
            ...
            if self.position != FLAT:
                ...
                # 수익이 발생한 경우 더 큰 보상
                if profit > 0:
                    reward += profit_rate * 20  # 수익에 비례한 보상 증가
                    self.success += 1
                    trade_success = True  # 거래 성공
                else:
                    reward += profit_rate * 15  # 손실에 비례한 페널티 증가
                    self.failure += 1
                
                # 연속 손실에 대한 추가 페널티
                if profit < 0 and self.failure > 1:
                    reward -= 12  # 연속 손실에 대한 페널티 증가
            
            self.balance_profit_rate = (self.balance - self.initial_balance) / self.initial_balance 
            
            # 청산 조건 - 큰 손실 방지
            if self.balance_profit_rate < -0.015:  # 손실 허용 범위 감소
                reward -= 40  # 큰 손실에 대한 페널티 증가
                #done = True
                #self.liquidated = True
            # 목표 달성 조건 - 수익 실현
            elif self.balance_profit_rate > 0.03:  # 목표 수익률 감소
                reward += 25  # 목표 달성에 대한 보상 증가
                #done = True
                #self.clear = True
        ...
        if done:
            ...
            if final_profit_rate > 0:
                reward += 25  # 수익에 대한 보상 증가
            elif final_profit_rate < -0.015:  # 손실 허용 범위 감소
                reward -= 30  # 손실에 대한 페널티 증가
        else:
            self.current_step += 1
