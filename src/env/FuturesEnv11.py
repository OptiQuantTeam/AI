...
class FuturesEnv11(gym.Env):
    ...
    def reset(self):
        ...
        if self.recurrence < 2 and self.recurrence > 0:
            self.current_step = self.tmp_current
            self.recurrence += 1
        else:
            self.current_step = np.random.randint(36, len(self.data) - self.max_steps)
            self.recurrence = 0 if self.recurrence == 5 else self.recurrence + 1
    ...
    def step(self, action):
        ...
        # 1. 행동에 대한 즉각적인 보상
        if action == LONG:
            action_reward -= 0.0  # LONG 진입 시도에 대한 작은 보상
        elif action == SHORT:
            action_reward -= 0.0  # SHORT 진입 시도에 대한 작은 보상
        else:
        ...
        # 포지션 유지 중일 때 손절매 로직 체크
            ...
            if unrealized_profit < -self.stop_loss_threshold:
                ...
                if self.consecutive_losses > 0:
                    exit_reward -= 0.2 * self.consecutive_losses
            ...
            # 익절매 로직
                ...
                if self.consecutive_wins > 0:
                    exit_reward += 0.2 * self.consecutive_wins
                action_reward += 5  # 익절매 실행에 대한 보상 (리스크 관리)
                
                # 수익 기록
                self.success += 1

            # 레버리지 조정 (연속 손실/수익 카운터도 여기서 처리)
            self._adjust_leverage(profit_rate)
